	...
	while ( N_Chars != 0 ) {
		if ( (*B_Flag == 1) && ((*Count + N_Chars + 1) <= LIMIT) ) {
			...
			*/
			...
			putchar(' ');
			*B_Flag = 0;
			for ( i = Start; i <= Start + N_Chars - 1; i++ ) { //부등호
				putchar(line[i]);
			}
			Start = Start + N_Chars;
			*Count = *Count + N_Chars + 1;
			...
			for ( i = Start; i <= Start + N_Blanks + N_Chars - 1; i++ ) { //부등호
				putchar(line[i]);
			}
			...
			*Count = *Count + N_Blanks + N_Chars;
			...
			if ( *Count == 0 ) {
				...
				Start = Start + N_Blanks + N_Chars;
				*Count = N_Chars + N_Blanks; //수정
				Get_Blanks_Chars( line, Start, &N_Blanks, &N_Chars );
			}
			else {
				putchar('\n');
				*B_Flag = 0;
				for ( i = Start + N_Blanks; i <= Start + N_Blanks + N_Chars - 1; i++ ) { //부등호
					putchar(line[i]);
				}
				Start = Start + N_Blanks + N_Chars;
				*Count = N_Chars;
